...
def add_task():
    data = request.json
    new_task = {"description": data["description"], "status": "Pending"}
...
def add_task_gui():
    desc = entry.get()
    if desc:
        tasks.append({"description": desc, "status": "Pending"})
        save_tasks(tasks)
...
add_button = tk.Button(frame, text="Add", font=("Arial", 12), bg="#007BFF", fg="white", command=add_task_gui)
